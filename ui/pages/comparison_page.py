...
def render_comparison_page():
    ...
    st.markdown("## :material/experiment: So Sánh Nhóm — T-test / ANOVA")
    ...
    if len(all_cols) < 2:
        st.warning("Cần ít nhất 2 cột: 1 cột nhóm + 1 cột giá trị.")
        return
    ...
    # ═══════════════════════════════════════════════════════════════
    st.markdown(section_header("Bước 2 — Cấu trúc nhóm", "groups"), unsafe_allow_html=True)
    ...
    with col_b:
        if n_groups_total == 2:
            ...
            st.info(f"**{n_groups_total} nhóm** → Nên dùng:\n**ANOVA** hoặc **Kruskal-Wallis**")
        else:
            st.warning(f"Chỉ có {n_groups_total} nhóm.")
    ...
    # ═══════════════════════════════════════════════════════════════
    st.markdown(section_header("Bước 3 — Chọn kiểm định", "science"), unsafe_allow_html=True)
    ...
    if test_key in _TEST_INFO:
        ...
        with st.expander(f"{info['title']} — Giải thích chi tiết", expanded=False, icon=":material/help:"):
            ...
            if "condition" in info:
                ...
                if cond == 2 and n_eff > 2:
                    st.warning(f"{info['title']} phù hợp cho 2 nhóm. Bạn đang có {n_eff} nhóm → nên dùng ANOVA/Kruskal-Wallis.")
                elif cond == 3 and n_eff < 3:
                    st.info(f"{info['title']} thường dùng cho ≥ 3 nhóm. Với 2 nhóm, T-test/Mann-Whitney hiệu quả hơn.")
    ...
    st.markdown("---")
    if st.button("Chạy kiểm định", type="primary", use_container_width=True, key="cmp_run", icon=":material/play_arrow:"):
...
def _render_results(data: dict, result):
    ...
    # A — KẾT QUẢ TỔNG QUAN
    # ═══════════════════════════════════════════════════════════════
    st.markdown(section_header("Kết Quả Tổng Quan", "assessment"), unsafe_allow_html=True)
...
- p-value nhỏ không đồng nghĩa với khác biệt lớn; cần xem effect size.
    ...
    if group_stats:
        st.markdown(section_header("A. Group Statistics (Thống kê mô tả)", "table_view"), unsafe_allow_html=True)
        ...
        if "Mean" in gs_df.columns:
            ...
            st.caption("Nhóm tô xanh có Mean cao nhất | Nhóm tô vàng có Mean thấp nhất")
        else:
            st.dataframe(gs_df, use_container_width=True, hide_index=True)

        with st.expander("Giải thích thống kê nhóm", expanded=False, icon=":material/help:"):
            st.markdown("""
...
So sánh Mean và Median: nếu khác biệt lớn, dữ liệu có thể lệch (skewed).
    ...
    if assumptions:
        ...
        with st.expander("Kiểm Tra Giả Định (Normality & Equal Variance)", expanded=True, icon=":material/fact_check:"):
            st.markdown("""
...
> Nếu vi phạm, nên dùng phiên bản phi tham số (Mann-Whitney, Kruskal-Wallis).
            ...
            # ── Normality ──
            if normality:
                st.markdown("#### :material/experiment: Kiểm định Shapiro-Wilk (Phân phối chuẩn)")
                norm_rows = []
                ...
                def highlight_norm(row):
                    if str(row["Phân phối chuẩn?"]) == "Không chuẩn":
                        return ["background-color: #fff3cd"] * len(row)
                    return ["background-color: #d4edda"] * len(row)
                ...
                if all_normal:
                    ...
                    st.warning("Một hoặc nhiều nhóm KHÔNG đạt phân phối chuẩn (p ≤ 0.05). Nên dùng Mann-Whitney U hoặc Kruskal-Wallis.")

                with st.expander("Hiểu về Shapiro-Wilk", expanded=False, icon=":material/help:"):
                    st.markdown("""
**Shapiro-Wilk Test:**
...
Trong trường hợp này, xem thêm biểu đồ Q-Q hoặc dùng test phi tham số cho an toàn.
            ...
            # ── Equal Variance ──
            if ev:
                st.markdown("#### :material/balance: Kiểm định Levene (Đồng nhất phương sai)")
                ...
                if has_equal_var:
                    ...
                    st.warning("Phương sai KHÔNG đồng nhất (p ≤ 0.05). T-test vẫn chạy được với Welch correction; ANOVA nên thay bằng Kruskal-Wallis.")

                with st.expander("Hiểu về Levene Test", expanded=False, icon=":material/help:"):
                    st.markdown("""
**Levene's Test:**
...
- ANOVA: Xem xét chuyển sang Kruskal-Wallis hoặc Welch ANOVA.
    ...
    if ttest_details:
        st.markdown(section_header("B. Independent Samples Test", "science"), unsafe_allow_html=True)
        ...
        st.caption("Dòng được **tô viền xanh/in đậm** là kết quả nên chọn dựa trên kiểm định phương sai Levene.")
        
    else:
        # Fallback for Mann-Whitney / Kruskal / etc
        mean_diff = data.get("mean_difference")
        if mean_diff is not None:
            st.markdown(section_header("Chi Tiết Kiểm Định Tương Đương", "straighten"), unsafe_allow_html=True)
    ...
    if post_hoc:
        st.markdown(section_header("Post Hoc — So Sánh Từng Cặp Nhóm", "compare_arrows"), unsafe_allow_html=True)
        ...
        if ph_results:
            ...
            with st.expander("Giải thích Post Hoc", icon=":material/help:"):
                st.markdown(f"""
...
- **95% CI**: Khoảng tin cậy 95% của chênh lệch. Nếu không chứa 0 → khác biệt có ý nghĩa.
    ...
    # D — KẾT LUẬN & GỢI Ý
    # ═══════════════════════════════════════════════════════════════
    st.markdown(section_header("Kết Luận & Gợi Ý", "lightbulb"), unsafe_allow_html=True)
...
- Có thể do: cỡ mẫu nhỏ, biến thiên trong nhóm lớn, hoặc thực sự không khác biệt.
    ...
    if st.button("Phân tích chuyên sâu bằng AI", key="ai_cmp_btn", icon=":material/auto_awesome:"):
...
def _render_effect_guide(effect_label: str, effect, effect_interp: str):
    ...
    rows = []
    for rng, label, meaning in guide["rows"]:
